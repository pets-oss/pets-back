    ...
    Mutation: {
      ...
      updateOrganization: async (_, { input }, { pgClient }) => {
        if (Object.keys(input).length < 2) {
          throw new Error('You have to provide at least one data field when updating an entity');
        }
      ...
      deleteOrganization: async (_, { id }, { pgClient }) => {
        const getDeleteTimeResponse = await pgClient.query(getDeleteTimeQuery(id));
