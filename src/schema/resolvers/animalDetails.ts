    ...
    AnimalDetails: {
        gender: async ({ gender_id }, { language }, { pgClient }) => {
            const dbResponse = await pgClient.query(
                getGenderQuery(gender_id, language, defaultLanguage)
            );
        ...
        breed: async ({ breed_id }, { language }, { pgClient }) => {
            const dbResponse = await pgClient.query(
                getBreedQuery(breed_id, language, defaultLanguage)
            );
        ...
        species: async ({ breed_id }, { language }, { pgClient }) => {
            const dbResponse = await pgClient.query(
                getSpeciesByBreedIdQuery(breed_id, language, defaultLanguage)
            );
        ...
        color: async ({ color_id }, { language }, { pgClient }) => {
            const dbResponse = await pgClient.query(
                getColorQuery(color_id, language, defaultLanguage)
            );
