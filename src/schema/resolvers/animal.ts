  ...
  Mutation: {
      ...
      try {
        ...
        if (input.microchip) {
          createMicrochipResult = await pgClient.query(
              createAnimalMicrochipQuery({...input.microchip, animalId})
          )
        }
    ...
    updateAnimal: async (_, { input }, { pgClient }) => {
      try {
        ...
        const updateDetailsResult = await getUpdateDetailsResult(input, pgClient);
        const updateMicrochipResult = await getUpdateMicrochipResult(input, pgClient);
